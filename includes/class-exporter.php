...
class WP_Data_Bridge_Exporter {
    ...
    public function export_site_data(int $site_id, array $options): array {
        ...
        try {
            ...
            if (!empty($post_types)) {
                ...
                if ($include_images && !empty($posts_data)) {
                    $post_ids = wp_list_pluck($posts_data, 'post_id');
                    $images_data = $this->get_featured_images_data($post_ids);
    ...
    public function get_featured_images_data(array $post_ids): array {
        if (empty($post_ids)) {
            return [];
        }
        ...
        foreach ($post_ids as $post_id) {
            ...
            $file_size = $image_path && file_exists($image_path) ? filesize($image_path) : 0;
            ...
            $images_data[] = [
                ...
                'associated_posts' => implode(', ', $associated_posts)
            ];
        }
